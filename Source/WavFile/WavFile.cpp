...
void WavFile::Read(uint8_t* buffer, size_t data_size) {
	if (m_file_header.byte_rate % data_size) {
...
void WavFile::CompleteFile() {
	if (m_file_open_mode == OPEN_MODE::WRITE) {
		m_file_header.wav_size = static_cast<uint32_t>(m_total_data_size
			- (sizeof(m_file_header.riff_sign) + sizeof(m_file_header.wav_size))
			+ sizeof(m_file_header));

		m_wav_file.seekp(offsetof(WavFile::WAV_FILE_HEADER, wav_size), m_wav_file.beg);
		m_wav_file.write(reinterpret_cast<const char*>(&m_file_header.wav_size), sizeof(m_file_header.wav_size));

		m_file_header.data_bytes = static_cast<uint32_t>(m_total_data_size);
...
uint32_t WavFile::GetDataSizeInBytes() const noexcept {
	return m_file_header.data_bytes;
}